...
const lastBraceIndex = mainPortalContent.lastIndexOf('};');
if (lastBraceIndex !== -1) {
    mainPortalContent = mainPortalContent.slice(0, lastBraceIndex) + privacyRoute + '\n};\n';
    fs.writeFileSync(mainPortalPath, mainPortalContent, 'utf8');
    console.log('Patched mainPortal.js safely');
} else {
