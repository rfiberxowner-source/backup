...
db.collection('push_notifications').where('status', '==', 'pending').onSnapshot(async (snap) => {
    for (const docSnap of snap.docs) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log(`Push notification sent to ${data.to}`);
            } else {
                const errText = await response.text();
                await docSnap.ref.update({ status: 'failed', error: errText });
                console.error(`Push notification failed for ${data.to}:`, errText);
            }
        } catch (error) {
            console.error('Error processing push notification:', error);
            await docSnap.ref.update({ status: 'failed', error: error.message });
        }
    }
}, (error) => {
    console.error("Error listening to push_notifications:", error);
});
